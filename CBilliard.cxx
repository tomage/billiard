...
CBilliard::CBilliard() {
	...
	holes = new CMatrix[6];
	holes[0] = CMatrix(3,1);
	holes[0].setEntry(0,0, dTABLE_SIZEX/2 - dHOLE_SIZE*dHoleVariance); holes[0].setEntry(1,0, dTABLE_SIZEY/2 - dHOLE_SIZE*dHoleVariance);
	holes[1] = CMatrix(3,1);
	holes[1].setEntry(0,0, dTABLE_SIZEX/2 - dHOLE_SIZE*dHoleVariance); holes[1].setEntry(1,0,-dTABLE_SIZEY/2 + dHOLE_SIZE*dHoleVariance);
	holes[2] = CMatrix(3,1);
	holes[2].setEntry(0,0,-dTABLE_SIZEX/2 + dHOLE_SIZE*dHoleVariance); holes[2].setEntry(1,0, dTABLE_SIZEY/2 - dHOLE_SIZE*dHoleVariance);
	holes[3] = CMatrix(3,1);
	holes[3].setEntry(0,0,-dTABLE_SIZEX/2 + dHOLE_SIZE*dHoleVariance); holes[3].setEntry(1,0,-dTABLE_SIZEY/2 + dHOLE_SIZE*dHoleVariance);
	holes[4] = CMatrix(3,1);
	holes[4].setEntry(1,0, dTABLE_SIZEY/2 - dHOLE_SIZE*dHoleVariance*dHoleVariance*dHoleVariance*dHoleVariance);
	holes[5] = CMatrix(3,1);
	holes[5].setEntry(1,0,-dTABLE_SIZEY/2 + dHOLE_SIZE*dHoleVariance*dHoleVariance*dHoleVariance*dHoleVariance);
	...
	for (int i = 0; i < current->iNoBalls; i++) {
		current->uBalls[i]->x.setEntry(0,0, current->uBalls[i]->x.getEntry(0,0) + (double)rand()/(double)RAND_MAX/dBALL_SIZE*randfact);
		current->uBalls[i]->x.setEntry(1,0, current->uBalls[i]->x.getEntry(1,0) + (double)rand()/(double)RAND_MAX/dBALL_SIZE*randfact);
	}
...
// 	cout << "getX(): t = " << t << endl;
	temp.setEntry(0,0, b->v.getEntry(0,0) / dPHYSICS_K * (1 - exp(-dPHYSICS_K * t)));
	temp.setEntry(1,0, b->v.getEntry(1,0) / dPHYSICS_K * (1 - exp(-dPHYSICS_K * t)));
	temp.setEntry(2,0, b->v.getEntry(2,0) / dPHYSICS_K * (1 - exp(-dPHYSICS_K * t)));
...
double CBilliard::getCollisionTime(udtBall * a,  udtBall * b, double r1, double r2) {
	// Returns time of collision
	// This is a pretty complex solution to detect collision with two balls when either of them is stationary or moving. Need positions and velocity vectors. Is probably too heavy for checking all collisions, so better use some simpler methors before to check their interactivity.
	// NB: This model assumes a friction model of static only .... NEED MORE DESCRIPTION HERE...
	
	double k = dPHYSICS_K;
	
	double  x1 = a->x.getEntry(0,0),  y1 = a->x.getEntry(1,0),  z1 = a->x.getEntry(2,0);
	double  x2 = b->x.getEntry(0,0),  y2 = b->x.getEntry(1,0),  z2 = b->x.getEntry(2,0);
	double vx1 = a->v.getEntry(0,0), vy1 = a->v.getEntry(1,0), vz1 = a->v.getEntry(2,0);
	double vx2 = b->v.getEntry(0,0), vy2 = b->v.getEntry(1,0), vz2 = b->v.getEntry(2,0);
	
	double A =     (vx2-vx1)*(vx2-vx1) + (vy2-vy1)*(vy2-vy1) + (vz2-vz1)*(vz2-vz1);
	double B = -2.0*(k*(x2-x1)*(vx2-vx1) + (vx2-vx1)*(vx2-vx1) + k*(y2-y1)*(vy2-vy1) + (vy2-vy1)*(vy2-vy1) + k*(z2-z1)*(vz2-vz1) + (vz2-vz1)*(vz2-vz1));
	double C = k*k*(x2-x1)*(x2-x1)+(vx2-vx1)*(vx2-vx1)+2*k*(x2-x1)*(vx2-vx1) + k*k*(y2-y1)*(y2-y1)+(vy2-vy1)*(vy2-vy1)+2*k*(y2-y1)*(vy2-vy1) + k*k*(z2-z1)*(z2-z1)+(vz2-vz1)*(vz2-vz1)+2*k*(z2-z1)*(vz2-vz1);
	C -= (r1+r2)*(r1+r2)*(k*k); // Remember to subtract the right hand side of the equation: Ax^2 + Bx + C - R < 0
...
void CBilliard::getNextState() {
	...
	double dFullTime = -log((double)0.01) / dPHYSICS_K; // Full time of movement - afterwards, particle will stop no matter...
	...
	// 		cout << "a "; cb[i]->coll.print();
			...
			if (dWT0 > margin && dWT0 != -1) {
				if (dWT0 < dLeastTime) {
					dLeastTime = dWT0;
					cb[i]->dWallTime = dWT0;
					CMatrix m = getV(dWT0, cb[i]); m.setEntry(1,0, -m.getEntry(1,0) * dLossFactor);
					cb[i]->coll = m;
				}
			}
			...
			if (dWT1 > margin && dWT1 != -1) {
				if (dWT1 < dLeastTime) {
					dLeastTime = dWT1;
					cb[i]->dWallTime = dWT1;
					CMatrix m = getV(dWT1, cb[i]); m.setEntry(1,0, -m.getEntry(1,0) * dLossFactor);
					cb[i]->coll = m;
				}
			}
			...
			if (dWT2 > margin && dWT2 != -1) {
				if (dWT2 < dLeastTime) {
					dLeastTime = dWT2;
					cb[i]->dWallTime = dWT2;
					CMatrix m = getV(dWT2, cb[i]); m.setEntry(0,0, -m.getEntry(0,0) * dLossFactor);
					cb[i]->coll = m;
				}
			}
			...
			if (dWT3 > margin && dWT3 != -1) {
				if (dWT3 < dLeastTime) {
					dLeastTime = dWT3;
					cb[i]->dWallTime = dWT3;
					CMatrix m = getV(dWT3, cb[i]); m.setEntry(0,0, -m.getEntry(0,0) * dLossFactor);
					cb[i]->coll = m;
				}
			}
		}
	...
	// If not collision, move all balls toward 1% of the initial velocity applied to cueBall
	if (dLeastTime == dFullTime) {
		...
		for (int i = 0; i < current->iNoBalls; i++) {
			if (cb[i]->bActive) {
				if (cb[i]->dHoleTime == dLeastTime) {
					...
					cb[i]->coll = cb[i]->coll * getV(dLeastTime, cb[i]).len() / cb[i]->coll.len() * dLossFactor;
					...
					for (int j = i; j < current->iNoBalls; j++) {
						if (cb[i]->bActive) {
							if (cb[j]->dCollisionTime == dLeastTime && i != j) {
								...
								vec1.setEntry(0,0, nb[j]->x.getEntry(0,0) - nb[i]->x.getEntry(0,0));
								vec1.setEntry(0,1, nb[j]->x.getEntry(1,0) - nb[i]->x.getEntry(1,0));
								vec1.setEntry(0,2, nb[j]->x.getEntry(2,0) - nb[i]->x.getEntry(2,0));
								vec1 = vec1 / vec1.len(); // Normalize
								
								CMatrix vec2 = CMatrix(1,3);
								vec2.setEntry(0,0, nb[i]->x.getEntry(0,0) - nb[j]->x.getEntry(0,0));
								vec2.setEntry(0,1, nb[i]->x.getEntry(1,0) - nb[j]->x.getEntry(1,0));
								vec2.setEntry(0,2, nb[i]->x.getEntry(2,0) - nb[j]->x.getEntry(2,0));
								...
								// Now add together the original normal force, and the exchange of the parallel forces
								cb[i]->coll = v1 - p1 + p2;
								cb[j]->coll = v2 - p2 + p1;
		...
		for (int i = 0; i < current->iNoBalls; i++) {
			if (cb[i]->bActive) {
				if (cb[i]->dCollisionTime == dLeastTime || cb[i]->dWallTime == dLeastTime) {
					nb[i]->v = cb[i]->coll;
				}
			}
		}
...
void CBilliard::displayFunction () {
	...
	glPushMatrix();
		...
		for (int i = 0; i < 6; i++) {
			glPushMatrix();
				...
				glTranslated(holes[i].getEntry(0,0), holes[i].getEntry(1,0), holes[i].getEntry(2,0));
		...
		for (int i = 0; i < drawState->iNoBalls; i++) {
			if (drawState->uBalls[i]->bActive || false) {
				glPushMatrix();
					...
					glPushMatrix();
						glTranslated(pos.getEntry(0,0)*0.03, pos.getEntry(1,0)*0.03, -dBALL_SIZE*0.02);
					...
					if (false) {
						...
						glBegin(GL_LINE);
							...
							glVertex3d(vel.getEntry(0,0), vel.getEntry(1,0), vel.getEntry(2,0) + dBALL_SIZE);
						glEnd();
						
						CMatrix posnext = getX(drawState->endtime - drawState->time, b) - getX(time - drawState->time, b);
						glColor3d(0.0, 1.0, 0.0);
						glLineWidth(4.0);
						glBegin(GL_LINE);
							glVertex3d(0.0, 0.0, dBALL_SIZE);
							glVertex3d(posnext.getEntry(0,0), posnext.getEntry(1,0), posnext.getEntry(2,0) + dBALL_SIZE);
						glEnd();
	...
	if (bLookAtCue) {
		if (drawState->uCueBall->bActive) {
			...
			if (bLookTop) {
				double distance = 50;
				gluLookAt(cbpos.getEntry(0,0)-aim.getEntry(0,0)*distance, cbpos.getEntry(1,0)-aim.getEntry(1,0)*distance, cbpos.getEntry(2,0)-aim.getEntry(2,0)*distance + distance/2, cbpos.getEntry(0,0), cbpos.getEntry(1,0), cbpos.getEntry(2,0), 0.0, 0.0, 1.0);
			} else if (bLookCenter) {
				gluLookAt(0.0, 0.0, 40.0, cbpos.getEntry(0,0), cbpos.getEntry(1,0), cbpos.getEntry(2,0), 0.0, 0.0, 1.0);
			}
}
	}
...
void CBilliard::keyboardFunction (unsigned char key, int x, int y) {
	...
	switch(key)
	{
		...
		case 'E':
			current->uCueBall->v.setEntry(0,0, current->uCueBall->v.getEntry(0,0) + (double)0.5);
			current->uCueBall->v.setEntry(1,0, current->uCueBall->v.getEntry(1,0) + (double)-0.5);
			break;
		...
		case 'F':
			time+=timeStep;
			if (time > current->time) time = current->time;
			cout << "Time: " << time << " current time: " << current->time << " energy: " << energy(time) << endl;
			break;
		case 'v':	// Reset
		case 'V':
			time-=timeStep;
		...
		case 't':	// Reset
		case 'T':
			cout << " states:\t " << states << "\t endtime:\t " << states->endtime << endl;
			cout << "current:\t " << current << "\t   time:\t " << current->time << "\t endtime:\t " << current->endtime << endl;
			cout << "   draw:\t " << drawState << "\t   time:\t " << drawState->time << "\t endtime:\t " << drawState->endtime << endl;
			break;
		case 'z':
		case 'Z':
...
void CBilliard::reshapeFunction (int w, int h) {
	...
	glViewport(0,myOpenGL->iWindowHeight - myOpenGL->iScreenHeight ,(GLsizei)myOpenGL->iScreenWidth,(GLsizei)myOpenGL->iScreenHeight);
...
// 	cout << "fint: " << finaltime << "  ct: " << current->time << endl;
	...
	if (!bPause) {
		time += timeStep;
		if (time > current->endtime) time = current->endtime;
		
		if (drawState->endtime - time < timeStep/2 && (drawState != states || time > timeStep/2) && (drawState->next != current || timeStep/2 < current->endtime - time)) {
			bool bWall = false;
