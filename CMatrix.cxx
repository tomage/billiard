...
double CMatrix::getEntry(int i, int j) {
	return this->a[i][j];
}
...
CMatrix CMatrix::operator * (CMatrix _CMatrix) {
	...
	if (n != _CMatrix.m) {
		cout << "ERROR: Matrices not compatible! Product between " << m << "x" << n << " matrix and " << _CMatrix.m << "x" << _CMatrix.n << " matrix." << endl;
		return temp;
	}
...
CMatrix CMatrix::operator + (CMatrix _CMatrix) {
	...
	if (n != _CMatrix.n || m != _CMatrix.m) {
		cout << "ERROR: Matrices not compatible! + operator" << " n=" << n << " != _n=" << _CMatrix.n << " and m=" << m << " != _m=" << _CMatrix.m << endl;
		return temp;
	}
...
double *CMatrix::inverse (double *d) {
	double detA = d[0]*(d[4]*d[8] - d[5]*d[7]) + d[1]*(d[5]*d[6] - d[3]*d[8]) + d[2]*(d[3]*d[7] - d[4]*d[6]);
	if (detA != 0 && 1/detA != 0) {
